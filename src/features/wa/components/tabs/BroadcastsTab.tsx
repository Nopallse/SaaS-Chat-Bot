...
const BroadcastsTab = ({ onNewBroadcast: _onNewBroadcast }: BroadcastsTabProps) => {
  ...
  const fetchSessions = async () => {
    ...
    try {
      ...
      setSessions(data.filter((s) => s.connected));
    } catch (error: any) {
  ...
  const handleBroadcast = async (values: any) => {
    ...
    try {
      ...
      showSuccess(
        `Broadcast selesai! Sent: ${successCount}, Failed: ${failedCount}, Skipped: ${skippedCount}`
      );
  ...
  const historyColumns = [
    {
      ...
      key: 'type',
      render: (type: string) => (
        <Tag color={type === 'text' ? 'blue' : 'green'}>{type.toUpperCase()}</Tag>
      ),
    },
    {
      title: 'Session',
      ...
      children: (
        <Card bordered={false}>
          <Form form={form} layout="vertical" onFinish={handleBroadcast}>
            ...
            >
              <Select
                placeholder="Select connected WhatsApp session"
                loading={fetching}
                showSearch
            ...
            <Form.Item
              name="phones"
              label="Phone Numbers"
              rules={[{ required: true, message: 'At least 1 phone number required!' }]}
              tooltip="Enter phone numbers. Press Enter, comma, or space to add. You can also paste multiple numbers."
            >
              <PhoneNumberInput
            ...
            <Form.Item name="checkNumber" valuePropName="checked" initialValue={true}>
              <Checkbox>Check number (verify number exists on WhatsApp before sending)</Checkbox>
            </Form.Item>
    ...
    },
    {
      ...
      children: (
        <Table
          ...
          locale={{ emptyText: 'No broadcast history yet' }}
        />
      ),
    },
  ];
